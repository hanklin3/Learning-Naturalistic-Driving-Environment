        ...
        for i in range(len(vehicle_list)):
            ...
            if v.id not in self.pool.keys():
                self.pool[v.id] = {'vid': [str(v.id)], 'update': False, 'vehicle': [v], 'dt': [1], 't': [self.t_latest], 'missing_days': 0,
                                   'x': [v.location.x], 'y': [v.location.y], 'heading': [v.speed_heading_deg if hasattr(v, 'speed_heading_deg') else v.speed_heading],
                ...
                self.pool[v.id]['heading'].append(v.speed_heading_deg if hasattr(v, 'speed_heading_deg') else v.speed_heading)
    ...
    def flatten_trajectory(self, max_num_vehicles, time_length, output_vid=False):
        ...
        for _, traj in self.pool.items():
            ...
            for j in range(len(ts)):
                ...
                heading = np.radians(vs[j].speed_heading_deg if hasattr(vs[j], 'speed_heading_deg') else vs[j].speed_heading)  # Convert degrees to radians
                if lat is None:
                    continue
